...
public record FolderListResponse(@Schema(description = "폴더 목록") List<FolderResponse> folders) {
  ...
  public static FolderListResponse from(List<Folder> folders) {
    return new FolderListResponse(folders.stream().map(FolderResponse::from).toList());
  }

  @Schema(description = "폴더 단건 응답")
  public record FolderResponse(
      @Schema(description = "폴더 고유 ID", example = "1") Long folderId,
      @Schema(description = "폴더 이름 enum 값", example = "SOPT") String name,
